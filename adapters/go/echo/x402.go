...
func ParsePaymentToken(c echo.Context) *PaymentToken {
	tokenHeader := c.Request().Header.Get("X-Payment-Token")
	if tokenHeader == "" {
		return nil
	}
...
func Middleware(config Config) echo.MiddlewareFunc {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			return c.JSON(402, pr)
		}
	}
}

func RequirePayment(config Config) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			token := ParsePaymentToken(c)
			if token == nil || !token.IsValid() {
